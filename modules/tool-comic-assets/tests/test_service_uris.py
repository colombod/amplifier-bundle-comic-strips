...
async def test_list_assets_returns_uris(service, sample_png) -> None:
    ...
    await service.store_asset(
        "test-proj", "issue-001", "panel", "panel_01", source_path=sample_png
    )
    result = await service.list_assets("test-proj", "issue-001")
    assert len(result) == 1
    assert "uri" in result[0]
    assert result[0]["uri"].startswith(
        "comic://test-proj/issues/issue-001/panels/panel_01"
    )
...
async def test_list_characters_returns_uris(service, sample_png) -> None:
    ...
    assert len(result) == 1
    assert "uri" in result[0]
    assert result[0]["uri"].startswith("comic://test-proj/characters/explorer")
...
async def test_list_styles_returns_uris(service) -> None:
    ...
    assert len(result) == 1
    assert "uri" in result[0]
    assert result[0]["uri"].startswith("comic://test-proj/styles/manga")
